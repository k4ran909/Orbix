(() => {
  ...
  function findElementByorbixId(orbixId, runtimeId) {
    ...
    // Fall back to finding by orbix-id (will get first match)
    const escaped = CSS.escape(orbixId);
    return document.querySelector(`[data-orbix-id="${escaped}"]`);
  }
  ...
  function applyStyles(element, styles) {
    ...
    console.debug(
      `[orbix Visual Editor] Applying styles:`,
      styles,
      "to element:",
      element,
    );
  ...
  function handleGetStyles(data) {
    ...
    const element = findElementByorbixId(elementId, runtimeId);
    if (element) {
  ...
  function handleEnableTextEditing(data) {
    ...
    const element = findElementByorbixId(componentId, runtimeId);
    if (element) {
  ...
  function handleGetTextContent(data) {
    ...
    window.parent.postMessage(
      {
        type: "orbix-text-content-response",
        componentId,
  ...
  window.addEventListener("message", (e) => {
    ...
    switch (type) {
      case "get-orbix-component-styles":
        handleGetStyles(data);
        break;
      case "modify-orbix-component-styles":
        handleModifyStyles(data);
        break;
      case "enable-orbix-text-editing":
        handleEnableTextEditing(data);
        break;
      case "disable-orbix-text-editing":
        handleDisableTextEditing(data);
        break;
      case "get-orbix-text-content":
