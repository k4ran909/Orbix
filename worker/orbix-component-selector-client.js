(() => {
  const OVERLAY_CLASS = "__orbix_overlay__";
  let overlays = [];
  ...
  function updateOverlay(el, isSelected = false, isHighlighted = false) {
    ...
    const name = el.dataset.orbixName || "<unknown>";
    const file = (el.dataset.orbixId || "").split(":")[0];
    ...
    if (highlightedElement) {
      ...
      if (highlightedItem) {
        ...
        window.parent.postMessage(
          {
            type: "orbix-component-coordinates-updated",
            coordinates: {
              top: rect.top,
              left: rect.left,
              width: rect.width,
  ...
  function removeOverlayById(componentId) {
    ...
    overlays.forEach((item, index) => {
      if (item.el.dataset.orbixId === componentId) {
    ...
    if (
      highlightedElement &&
      highlightedElement.dataset.orbixId === componentId
    ) {
      highlightedElement = null;
    }
  }
  ...
  function onMouseMove(e) {
    ...
    let el = e.target;
    while (el && !el.dataset.orbixId) el = el.parentElement;
  ...
  function onClick(e) {
    ...
    const clickedComponentId = state.element.dataset.orbixId;
    ...
    if (selectedItem && (highlightedElement === state.element || !isProMode)) {
      ...
      window.parent.postMessage(
        {
          type: "orbix-component-deselected",
    ...
    if (!state.element.dataset.orbixRuntimeId) {
      state.element.dataset.orbixRuntimeId = `orbix-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    }

    const rect = state.element.getBoundingClientRect();
    window.parent.postMessage(
      {
        type: "orbix-component-selected",
        component: {
          id: clickedComponentId,
          name: state.element.dataset.orbixName,
          runtimeId: state.element.dataset.orbixRuntimeId,
        },
        coordinates: {
          top: rect.top,
          left: rect.left,
  ...
  function onKeyDown(e) {
    ...
    if (key === "c" && hasShift && hasCtrlOrMeta) {
      ...
      window.parent.postMessage(
        {
          type: "orbix-select-component-shortcut",
        },
        "*",
      );
    }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (e.data.type === "orbix-pro-mode") {
      isProMode = e.data.enabled;
    }
    if (e.data.type === "activate-orbix-component-selector") activate();
    if (e.data.type === "deactivate-orbix-component-selector") deactivate();
    if (e.data.type === "activate-orbix-visual-editing") {
      activate();
    }
    if (e.data.type === "deactivate-orbix-visual-editing") {
      deactivate();
      clearOverlays();
    }
    if (e.data.type === "clear-orbix-component-overlays") clearOverlays();
    if (e.data.type === "update-orbix-overlay-positions") {
    ...
    if (
      e.data.type === "remove-orbix-component-overlay" ||
      e.data.type === "deselect-orbix-component"
    ) {
  ...
  function initializeComponentSelector() {
    if (!document.body) {
      console.error(
        "orbix component selector initialization failed: document.body not found.",
      );
      return;
    }
    setTimeout(() => {
      if (document.body.querySelector("[data-orbix-id]")) {
        window.parent.postMessage(
          {
            type: "orbix-component-selector-initialized",
          },
          "*",
        );
        console.debug("orbix component selector initialized");
      } else {
        console.warn(
          "orbix component selector not initialized because no DOM elements were tagged",
        );
      }
    }, 0);
  }
