...
/**
 * A webpack loader that adds `data-Orbix-*` attributes to JSX elements.
 */
export default function OrbixTaggerLoader(this: any, code: string) {
  ...
  const transform = async () => {
    try {
      ...
      walk(ast as any, {
        enter: (node: any) => {
          try {
            ...
            if (!loc) return;
            const OrbixId = `${fileRelative}:${loc.line}:${loc.column}`;

            // Inject the attributes
            if (node.name.end != null) {
              ms.appendLeft(
                node.name.end,
                ` data-Orbix-id="${OrbixId}" data-Orbix-name="${tagName}"`,
              );
              transformCount++;
            }
          } catch (error) {
            console.warn(
              `[Orbix-tagger] Warning: Failed to process JSX node in ${this.resourcePath}:`,
              error,
            );
          }
        },
      ...
      console.warn(
        `[Orbix-tagger] Warning: Failed to transform ${this.resourcePath}:`,
        error,
      );
      return null;
    }
  ...
  transform()
    ...
    .catch((err) => {
      console.error(`[Orbix-tagger] ERROR in ${this.resourcePath}:`, err);
