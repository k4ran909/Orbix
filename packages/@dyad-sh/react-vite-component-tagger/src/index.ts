 ...
 */
export default function OrbixTagger(): Plugin {
  return {
    name: "vite-plugin-Orbix-tagger",
    apply: "serve",
    enforce: "pre",
    ...
    async transform(code: string, id: string) {
      try {
        ...
        walk(ast as any, {
          enter(node: any) {
            try {
              ...
              // ── 2. Check whether the tag already has data-Orbix-id ───────────────
              ...
              console.warn(
                `[Orbix-tagger] Warning: Failed to process JSX node in ${id}:`,
                error,
              );
            }
          },
        ...
        console.warn(
          `[Orbix-tagger] Warning: Failed to transform ${id}:`,
          error,
        );
        return null;
      }
