...
test("edit code", async ({ po }) => {
  const editedFilePath = path.join("src", "components", "made-with-Orbix.tsx");
  ...
  await po.page.getByText("made-with-Orbix.tsx").click();
  await po.page
    .getByRole("code")
    .locator("div")
  ...
  await po.page
    ...
    .fill("export const MadeWithOrbix = ;");
  ...
  expect(editedFile).toContain("export const MadeWithOrbix = ;");
});
