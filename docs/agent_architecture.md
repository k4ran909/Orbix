...
- `src/pro/main/ipc/handlers/local_agent/tool_definitions.ts` contains the list of all the tools available to the Orbix local agent.
...
Finally, you will need to define how to render the custom XML tag (e.g. `<orbix-$foo-tool-name>`) inside `src/components/chat/OrbixMarkdownParser.tsx` which will typically involve creating a new React component to render the custom XML tag.
