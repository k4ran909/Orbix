...
1. `Orbix-{version}-1.x86_64.rpm` (Linux RPM)
2. `Orbix-{version}-full.nupkg` (Windows NuGet package)
3. `Orbix-{version}.Setup.exe` (Windows installer)
4. `Orbix-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `Orbix-darwin-x64-{version}.zip` (macOS Intel)
6. `Orbix_{version}_amd64.deb` (Linux DEB)
