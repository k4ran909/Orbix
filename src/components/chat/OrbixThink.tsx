...
import { VanillaMarkdownParser } from "./OrbixMarkdownParser";
import { CustomTagState } from "./stateTypes";
import { OrbixTokenSavings } from "./OrbixTokenSavings";

interface OrbixThinkProps {
  node?: any;
  children?: React.ReactNode;
}

export const OrbixThink: React.FC<OrbixThinkProps> = ({ children, node }) => {
  ...
  const tokenSavingsMatch =
    typeof children === "string"
      ? children.match(
          /^Orbix-token-savings\?original-tokens=([0-9.]+)&smart-context-tokens=([0-9.]+)$/,
        )
      : null;
  ...
  // If it's token savings format, render OrbixTokenSavings component
