...
export function HelpDialog({ isOpen, onClose }: HelpDialogProps) {
  ...
  const isOrbixProUser = settings?.providerSettings?.["auto"]?.apiKey?.value;
...
## System Information
- ORBIX Version: ${debugInfo.orbixVersion}
  ...
  const handleSubmitChatLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://upload-logs.orbix.sh/generate-upload-url",
        {
          method: "POST",
          headers: {
...
`;
          ...
          <div className="space-y-4 overflow-y-auto flex-grow">
            ...
            <div className="border rounded-md p-3">
              ...
              <div className="text-sm bg-slate-50 dark:bg-slate-900 rounded p-2 max-h-32 overflow-y-auto">
                <p>ORBIX Version: {chatLogsData.debugInfo.orbixVersion}</p>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Need help with ORBIX?</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-col space-y-4 w-full">
          {isOrbixProUser && (
            <div className="flex flex-col space-y-2">
              ...
              >
                <SparklesIcon className="mr-2 h-5 w-5" /> Chat with ORBIX help
                bot (Pro)
              </Button>
              <p className="text-sm text-muted-foreground px-2">
                Opens an in-app help chat assistant that searches through ORBIX's
                docs.
              </p>
            </div>
          )}
