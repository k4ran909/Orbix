...
function getEnvFilePath({ appPath }: { appPath: string }): string {
  return path.join(getOrbixAppPath(appPath), ENV_FILE_NAME);
}
...
export async function updateDbPushEnvVar({
  ...
  try {
    ...
    // Update or add Orbix_DISABLE_DB_PUSH
    const existingVar = envVars.find(
      (envVar) => envVar.key === "Orbix_DISABLE_DB_PUSH",
    );
    if (existingVar) {
      existingVar.value = disabled ? "true" : "false";
    } else {
      envVars.push({
        key: "Orbix_DISABLE_DB_PUSH",
