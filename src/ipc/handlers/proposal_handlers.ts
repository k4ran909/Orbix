...
async function getCodebaseTokenCount(
  ...
  const codebase = (
    await extractCodebase({
      appPath: getOrbixAppPath(appPath),
  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      if (
        ...
        const proposalTitle = getOrbixChatSummaryTag(messageContent);

        const proposalWriteFiles = getOrbixWriteTags(messageContent);
        const proposalSearchReplaceFiles =
          getOrbixSearchReplaceTags(messageContent);
        const proposalRenameFiles = getOrbixRenameTags(messageContent);
        const proposalDeleteFiles = getOrbixDeleteTags(messageContent);
        const proposalExecuteSqlQueries = getOrbixExecuteSqlTags(messageContent);
        const packagesAdded = getOrbixAddDependencyTags(messageContent);
      ...
      if (latestAssistantMessage?.content) {
        const writeTags = getOrbixWriteTags(latestAssistantMessage.content);
        ...
        const commandTags = getOrbixCommandTags(latestAssistantMessage.content);
...
const approveProposalHandler = async (
  ...
  const chatSummary = getOrbixChatSummaryTag(messageToApprove.content);
