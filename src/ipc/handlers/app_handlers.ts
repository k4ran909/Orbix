...
import { getOrbixAppPath, getUserDataPath } from "../../paths/paths";
...
function listenToProcess({
  ...
  spawnedProcess.stdout?.on("data", async (data) => {
    ...
    // isNeon because: 1) only Neon apps (for the official Orbix templates) should
      ...
      if (urlMatch) {
        proxyWorker = await startProxy(urlMatch[1], {
          onStarted: (proxyUrl) => {
            safeSend(event.sender, "app:output", {
              type: "stdout",
              message: `[Orbix-proxy-server]started=[${proxyUrl}] original=[${urlMatch[1]}]`,
              appId,
            });
          },
        });
...
async function executeAppInDocker({
  ...
  const containerName = `Orbix-app-${appId}`;
  ...
  const dockerfilePath = path.join(appPath, "Dockerfile.Orbix");
...
`;
  ...
  const buildProcess = spawn(
    "docker",
    ["build", "-f", "Dockerfile.Orbix", "-t", `Orbix-app-${appId}`, "."],
    {
      cwd: appPath,
      stdio: "pipe",
    },
  ...
  const process = spawn(
    ...
    [
      ...
      `${appPath}:/app`,
      "-v",
      `Orbix-pnpm-${appId}:/app/.pnpm-store`,
      "-e",
...
export function registerAppHandlers() {
  ...
  handle(
    ...
    async (
      ...
      const fullAppPath = getOrbixAppPath(appPath);
  ...
  handle(
    ...
    async (_, params: CopyAppParams): Promise<{ app: any }> => {
      ...
      const originalAppPath = getOrbixAppPath(originalApp.path);
      const newAppPath = getOrbixAppPath(newAppName);
  ...
  ipcMain.handle("list-apps", async () => {
    ...
    return {
      apps: allApps,
      appBasePath: getOrbixAppPath("$APP_BASE_PATH"),
    };
  });
      ...
      return withLock(appId, async () => {
        try {
          ...
          if (removeNodeModules) {
            ...
            if (runtimeMode === "docker") {
              logger.log(
                `Docker mode detected for app ${appId}. Removing Docker volumes Orbix-pnpm-${appId}...`,
              );
              try {
                await removeDockerVolumesForApp(appId);
                logger.log(
                  `Removed Docker volumes for app ${appId} (Orbix-pnpm-${appId}).`,
                );
              } catch (e) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return withLock(appId, async () => {
        ...
        const oldAppPath = getOrbixAppPath(app.path);
        const newAppPath = getOrbixAppPath(appPath);
  ...
  ipcMain.handle("reset-all", async (): Promise<void> => {
    ...
    const OrbixAppPath = getOrbixAppPath(".");
    if (fs.existsSync(OrbixAppPath)) {
      await fsPromises.rm(OrbixAppPath, { recursive: true, force: true });
      // Recreate the base directory
      await fsPromises.mkdir(OrbixAppPath, { recursive: true });
    }
